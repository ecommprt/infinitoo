...
const CATEGORIES = [
  'Tendas', 'Cenografia', 'Iluminação', 'Som', 'Bandas',
  'Cozinha', 'Camarim', 'Figurino', 'Equipe', 'Restaurantes', 'Elétrica', 'Extras'
]
...
export default function SchedulePage() {
  ...
  const filtered = tasks?.filter(t =>
    ...
    (!filterResp || t.assignee === filterResp)
  ) ?? []
            ...
            {filtered.map((task, i) => {
              ...
              return (
                <tr
                  key={task.id}
                  className={clsx(
                    'border-b border-wilco-gray70/50 hover:bg-wilco-gray90/50 transition-colors',
                ...
                >
                  <td className="px-4 py-3">
                    ...
                    {isLate && (
                      <span className="ml-2 text-xs text-red-400 font-medium">ATRASADO</span>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <span className={clsx(
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="flex-1 bg-wilco-gray70 rounded-full h-1.5">
                        <div
                          className={clsx('h-1.5 rounded-full transition-all', isDone ? 'bg-green-500' : isLate ? 'bg-red-500' : 'bg-wilco-orange')}
