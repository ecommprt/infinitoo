...
export default function SchedulePage() {
    ...
    queryFn: async (): Promise<ScheduleTask[]> => {
      try {
        const { data, error } = await supabase
          .from('schedule_tasks')
          .select('id, category, title, assignee, start_date, end_date, pct_complete')
          .eq('event_id', eventId!)
          .order('start_date')

        if (error) {
          console.error('[Wilco/Cronograma] Erro:', error.message, error.details ?? '')
          return []
        }
        return data ?? []
      } catch (err) {
        console.error('[Wilco/Cronograma] Falha inesperada:', err)
        return []
      }
    },
    enabled: !!eventId
  ...
  const updatePct = useMutation({
    mutationFn: async ({ id, pct }: { id: string; pct: number }) => {
      const { error } = await supabase
        .from('schedule_tasks')
        .update({ pct_complete: pct })
        .eq('id', id)

      if (error) {
        console.error('[Wilco/Cronograma] Erro ao atualizar %:', error.message, error.details ?? '')
        throw error
      }
    },
  ...
  const late = filtered.filter(t =>
    new Date(t.end_date) < new Date() && t.pct_complete < 100
  ).length
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-wilco-gray50 text-sm mt-1">
          ...
          <span className="text-red-400">{late} em atraso</span>
        </p>
      </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-wilco-gray50 text-sm">Carregando cronograma...</p>
        </div>
      ) : (
            ...
            <tbody>
              {filtered.map((task, i) => {
                ...
                return (
                  <tr key={task.id}
                    className={clsx(
                      'border-b border-wilco-gray70/40 hover:bg-wilco-gray90/40 transition-colors',
                      i % 2 === 0 ? '' : 'bg-wilco-gray90/20'
                    )}>
                    <td className="px-4 py-3">
                      <span className={clsx('text-sm',
                        isDone ? 'text-wilco-gray50 line-through' : 'text-white')}>
                        {task.title}
                      </span>
                      {isLate && (
                    ...
                    <td className="px-4 py-3 text-wilco-gray20 text-xs">
                      {task.assignee ?? '—'}
                    </td>
                    <td className="px-4 py-3 text-wilco-gray50 text-xs">
                      {format(parseISO(task.start_date), 'dd/MM', { locale: ptBR })}
                    </td>
                    <td className={clsx('px-4 py-3 text-xs',
                      isLate ? 'text-red-400' : 'text-wilco-gray50')}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={e =>
                            updatePct.mutate({ id: task.id, pct: Number(e.target.value) })
                          }
                          className="flex-1 accent-wilco-orange h-1.5 cursor-pointer"
                        />
                        <span className="text-wilco-gray50 text-xs w-8 text-right">
                          {task.pct_complete}%
                        </span>
                      </div>
                    </td>
